...
[Multivariable linear regression](https://en.wikipedia.org/wiki/Linear_regression) can seem complicated, especially when including things like [factor variables](https://en.wikipedia.org/wiki/Categorical_variable) and [interactions](https://en.wikipedia.org/wiki/Interaction_(statistics)). In reality, however, it is relatively straightfoward to understand once you break it down into its components. In this blog post I will talk you through interpreting a linear regression model which includes one continuous variable, one factor variable and their interaction in R.

## Running the model

Let's consider a model using the `mtcars` dataset. In this model, we will predict `mpg` (miles per gallon) using `am` (transmission), `wt` (the car's weight in 1000lb increments) and their interaction term.

Before running the model, I will convert `am` into a factor variable. I could have also converted `am` to a factor within the model using `factor(am)`, but I wanted to label each level to make the explanation of the coefficients more clear:
...
You can see from the above, this model has four coefficients, the intercept, the main effect for `wt`, the main effect for `am` and the interaction of `wt * am`. So how do we interpret these? As you will recall, a regression equation takes the form:

$$E[Y | X] = \beta_0 + \beta_1X$$

indicating that the expected value of $Y$, given the value of $X$, is given by the intercept plus the value of $X$ multiplied by $\beta_1$. Our model is therefore:

$$E[Y | X_1X_2] = \beta_0 + \beta_1(weight) + \beta_2(transmission) + \beta_3(weight \cdot transmission)$$

Ok, so how do we interpret this? The first part is to understand that the intercept is the miles per gallon a car is expected to have when both car weight and car transmission are equal to 0 (or when the car is automatic). This can be seen when we substitute $X_1$ and $X_2$ for 0:
...
In this case the intercept, or the miles per gallon of an automatic car of weight 0lbs, is `r round(summary(model)$coef[1,1], 2)`. Obviously this is a useless number as no real car can have a weight of 0. We can make our coefficients more useful using a technique called **centring**.

## Centring makes interpretation easier

Centring involves subtracting the mean from the continuous predictors in the model in order to reset the point at which the regression slope crosses the intercept to be the mean. This is demonstrated in the figure below:
...
You can see that centring replaces the true intercept value for weight (0 lbs, a non-existant value) with a new intercept value (`r round(mean(mtcars$wt), 2)`) by making the model consider the mean as `wt` = 0. Let's try and interpret our intercept again using our centred weight variable. Firstly, we need to create a new model:
...
Now that we've covered the intercept, how do we interpret the rest? The trick is to break this down by the different levels of `am`. When the transmission is automatic (or when $X_2$ = 0), the regression equation breaks down like so:
...
for cars with manual transmission. So our model is telling us that a 2620 lb car (which would have a value of `r round(2.620 - mean(mtcars$wt), 2)` in our centred weight variable) would is predicted to have an MPG of `r round(summary(centred.model)$coef[1,1] + summary(centred.model)$coef[2,1] * (2.620 - mean(mtcars$wt)), 2)` if it was automatic and `r round((summary(centred.model)$coef[1,1] + summary(centred.model)$coef[3,1]) + (summary(centred.model)$coef[2,1] + summary(centred.model)$coef[4,1]) * (2.620 - mean(mtcars$wt)), 2)` if it were manual.

When comparing the two models, you may have noticed that the two equations differ in both their intercept and slope values. More specifically, $\beta_2$ is the change in the **intercept** when changing from automatic to manual transmission, while $\beta_3$ represents the change in **slope** changing from automatic to manual transmission. In other words, at the mean car weight, manual cars get `r round(abs(summary(centred.model)$coef[3,1]), 2)` less miles per gallon than automatic cars. In addition, manual cars get `r round(abs(summary(centred.model)$coef[4,1]), 2)` less miles per gallon for every addition 1000 lbs in weight. You can see this in the figure below. Note that the interaction terms cross, meaning that manual cars actually gain `r round(abs(summary(centred.model)$coef[4,1]), 2)` miles per gallon for every 1000 lbs less of car weight.
...
The final part of interpreting regression coefficients is getting an estimate of their precision. We will do this using confidence intervals. We can shortcut this process using R's inbuilt `predict` function. The first thing we need to do is create separate models for the automatic and manual transmissions.
...
We'll then construct a new dataframe containing the values of weight we want to put a confidence interval around. Let's go with 2620 lbs, which corresponds to a manual car in our dataset, the Mazda RX4, and 3570 lbs, which corresponds with the Duster 360, an automatic car in our dataset. I've picked a 95% confidence interval (which is also the default in the predict function).
...
You can see the predicted MPG for the 2000 lb manual car is `r round(manual.predict.ci[ ,1], 2)` [95% prediction interval: `r round(manual.predict.ci[ ,2], 2)`, `r round(manual.predict.ci[ ,3], 2)`], and `r round(auto.predict.ci[ ,1], 2)` [95% prediction interval: `r round(auto.predict.ci[ ,2], 2)`, `r round(auto.predict.ci[ ,3], 2)`] for the automatic car. You can see that these intervals are significantly wider than the confidence interval.
